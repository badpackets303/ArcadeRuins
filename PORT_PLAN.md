...
| **X4** | Installers (pkg / Windows installer / tarball), signing (notarisation reused; Windows Authenticode is the owner's certificate), host matrix, docs, release | Tagged release, checksums, goldens green on the release commit. **Done 2026-09-22 — v1.0.0**, except: the Windows installer is UNSIGNED by decision (ADR-097), the Intel half of the macOS build ships untested (ADR-095), and **X4-3's host matrix is one row — Logic Pro, by hand; the owner has no other DAW** and asks users for the rest. ADR-094 (the three packagers), ADR-098 (one public repository). |
