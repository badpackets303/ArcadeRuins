...
open class AKTuningTableBase: NSObject {
    ...
    public override init() {
        ...
        for noteNumber in 0..<AKTuningTable.midiNoteCount {
            // PORT FIX: upstream reads `exp2((noteNumber - 69) / 12)`, which does
            // not compile under a modern Swift — noteNumber is Int, so the
            // subexpression is integer division and there is no exp2(Int).
            // Converted to Double, which is also the mathematically correct
            // reading. AKTuningTable overrides init() and calls defaultTuning(),
            // so this base-class table is overwritten for the subclass Synth One
            // uses; it is fixed here so the base class is correct on its own.
            // See Sources/S1Support/PORTING.md.
