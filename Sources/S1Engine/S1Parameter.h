...
// PORT FIX (P1-6): a named include guard. S1Parameter.h is a public header of
// SynthOneCore, and Xcode flattens public headers into Headers/, so the file is
// legitimately reached by two different paths inside the framework build — the
// source tree for the kernel, and the copied public header for anything that comes
// in through <SynthOneCore/...>. `#pragma once` keys on file identity, so it would
// let the enum be defined twice; a macro guard does not.
//
// PORT (X1-3, ADR-067): self-contained. This used to import <SynthOneCore/AKInterop.h>
// for AK_ENUM; the portable engine (Sources/S1Engine) includes this header from a tree
// with no SynthOneCore in it, and GCC and MSVC have no `enum_extensibility`. Under
// Clang the enum is exactly what AK_ENUM made it — an open enum, which is how Swift
// has always seen it — and elsewhere a plain int-backed enum.
#ifndef S1_PARAMETER_H
#define S1_PARAMETER_H

#if defined(__clang__)
#define S1_PARAMETER_ENUM(name) enum __attribute__((enum_extensibility(open))) name : int
#else
#define S1_PARAMETER_ENUM(name) enum name : int
#endif

typedef S1_PARAMETER_ENUM(S1Parameter) {
    index1 = 0,
    index2 = 1,
    morphBalance = 2,
