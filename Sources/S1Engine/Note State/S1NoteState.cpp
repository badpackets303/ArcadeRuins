...
void S1NoteState::run(int frameIndex, float *outL, float *outR) {
    ...
    //oscmorph1_out
    sp_oscmorph2d_compute(kernel->spp(), oscmorph1, nullptr, &oscmorph1_out);
    oscmorph1_out *= getParam(morph1Volume);
    
    //oscmorph2_out
    sp_oscmorph2d_compute(kernel->spp(), oscmorph2, nullptr, &oscmorph2_out);
    ...
    //subOsc_out
    sp_osc_compute(kernel->spp(), subOsc, nullptr, &subOsc_out);
    ...
    //fmOsc_out
    sp_fosc_compute(kernel->spp(), fmOsc, nullptr, &fmOsc_out);
    fmOsc_out *= getParam(fmVolume);
    
    //noise_out
    sp_noise_compute(kernel->spp(), noise, nullptr, &noise_out);
