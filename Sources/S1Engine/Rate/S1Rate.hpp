...
#include "AKSynthOneRate.h"
#include <cmath>
#include <string>
// PORT FIX (X1-5, ADR-069): the three "nearest rate" searches below said `abs(float)`. Under
// Apple's libc++ and MSVC that finds a float overload; under libstdc++ it finds C's `abs(int)`,
// which truncates the difference to a whole number and picks the wrong tempo-synced rate. The
// Linux job of the engine workflow showed it as 2,513 mismatching parameter values across the
// factory presets (lfo1Rate, lfo2Rate, autoPanFrequency, delayTime, arpSeqTempoMultiplier).
// `std::fabs` is what the Mac has always computed.
...
public:
    ...
    inline S1RateArgs nearestFrequency(float inputFrequency, float inputTempoBPM, float min, float max) {
        ...
        for(int i = eightBars; i < AKSynthOneRate::AKSynthOneRateCount; i++) {
            ...
            if (tempoAsFrequency < min || tempoAsFrequency > max)
                continue;
            float difference = std::fabs(tempoAsFrequency - inputFrequency);   // PORT FIX (X1-5): was `abs`, see below
    ...
    inline S1RateArgs nearestTime(float inputTime, float inputTempoBPM, float min, float max) {
        ...
        for(int i = sixtyFourthTriplet; i >= twoBars; i--) {
            ...
            if (tempoTime < min || tempoTime > max)
                continue;
            float difference = std::fabs(tempoTime - inputTime);   // PORT FIX (X1-5)
            if (difference < smallestDifference) {
                smallestDifference = difference;
                closestRate = i;
    ...
    inline S1RateArgs nearestFactor(float inputFactor) {
        ...
        for(int i = sixtyFourthTriplet; i >= eightBars; i--) {
            ...
            float difference = std::fabs(factor - inputFactor);   // PORT FIX (X1-5)
            if (difference < smallestDifference) {
                smallestDifference = difference;
                closestRate = i;
