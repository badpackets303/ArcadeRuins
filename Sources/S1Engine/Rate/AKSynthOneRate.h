...
// PORT (X1-3): was `typedef AK_ENUM(AKSynthOneRate)`, which needs AKInterop.h and Clang.
// Only C++ reads this header, so a plain int-backed enum is the same thing.
typedef enum AKSynthOneRate : int {
    eightBars = 0,
    sixBars = 1,
    fourBars = 2,
    threeBars = 3,
