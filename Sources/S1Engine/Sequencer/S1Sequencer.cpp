...
#include <algorithm>   // PORT (X1-3): std::for_each
#include <cmath>       // PORT (X1-3): fmod
...
// PORT: net-new at P4-5. See the header for why `reset(bool)` is not this.
void S1Sequencer::resetPosition() {
    mBeatTime = 0;
    mStepCounter = 0;
}

void S1Sequencer::process(DSPParameters &params, const S1HeldNoteList &heldNotes) {
    
    /// MARK: ARPEGGIATOR + SEQUENCER BEGIN
    const int heldNoteNumbersAECount = heldNotes.count;
            ...
            if (arpSeqIsOn) {
                ...
                if (heldNoteNumbersAECount > 0) {
                    ...
                    if ( params[arpIsSequencer] == 1.f ) {
                        ...
                        // ARPEGGIATOR
                        for (const NoteNumber &note : heldNotes) {
                    ...
                    if ( sequencerNotes.size() > 0 ) {
                        ...
                        if (params[arpIsSequencer] == 1.f) {
                            ...
                            // SEQUENCER
                            if (snn.onOff == 1) {
                                for (const NoteNumber &noteStruct : heldNotes) {
