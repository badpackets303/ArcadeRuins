...
// PORT (X1-3): Objective-C's `nullptr` for the unused input of the Soundpipe compute calls is
// `nullptr` here — the same null, spelled in C++.
#include "S1NoteState.hpp"

void S1DSPKernel::freeReleasedVoices() {
...
void S1DSPKernel::process(S1FrameCount frameCount, S1FrameCount bufferOffset) S1_NONBLOCKING {
    initializeNoteStates();

    // PREPARE FOR RENDER LOOP...updates here happen at 44100/frameCount Hz
    // PORT (X1-3): was `(float*)outBufferListPtr->mBuffers[0].mData`; the adapter sets these.
    float* outL = outputLeft + bufferOffset;
    float* outR = outputRight + bufferOffset;

    // currently UI is visible in DEV panel only so can't be portamento


    /// transition playing notes from release to off
    // PORT (X2-3, ADR-074): upstream does this here, once per call — so WHEN a released voice is
    // freed depends on where the host's buffers fall, and with it whether a retriggered note finds
    // its old voice still sounding (turnOnKey reuses it, phase and filter state and all) or takes a
    // fresh one. An arpeggio with a short release renders differently at every buffer size. A host
    // that must not depend on its buffer size sets `freeReleasedVoicesEveryFrame`, which moves the
    // same check to the top of every frame: exactly what upstream renders in one-frame buffers.
    // Off, this is upstream's code path and the goldens' (512-frame buffers).
    if (!freeReleasedVoicesEveryFrame) {
        freeReleasedVoices();
    }
    ...
    for (S1FrameCount frameIndex = 0; frameIndex < frameCount; ++frameIndex) {
        ...
        sp_phasor_compute(sp, lfo1Phasor, nullptr, &lfo1); // sp_phasor_compute [0,1]
        ...
        sp_phasor_compute(sp, lfo2Phasor, nullptr, &lfo2);  // sp_phasor_compute [0,1]
        ...
        sequencer.process(parameters, heldNotes.snapshot());
        ...
        float panValue = 0.f;
        sp_osc_compute(sp, panOscillator, nullptr, &panValue);
        pan->pan = panValue;
