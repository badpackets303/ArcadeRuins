...
void S1DSPKernel::turnOffKey(int noteNumber) {
    ...
    if (parameters[isMono] > 0.f) {
        ...
        // MONO:
        // PORT (X1-2): one snapshot serves the count and the head, where AEArray read them apart.
        const S1HeldNoteList held = heldNotes.snapshot();
        if (parameters[arpIsOn] == 1.f || held.count == 0) {
            ...
            const NoteNumber* nn = &held.notes[0];
