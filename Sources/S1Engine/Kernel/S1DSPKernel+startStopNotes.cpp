...
void S1DSPKernel::startNote(int noteNumber, int velocity, float frequency) {
    ...
    // PORT (X1-2): remove any earlier press of this key, then insert it first — as the
    // NSMutableArray dance here did.
    heldNotes.noteOn(noteNumber, (int)parameters[transpose], velocity);
