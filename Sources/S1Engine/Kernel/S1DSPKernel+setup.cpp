...
void S1DSPKernel::setupWaveform(uint32_t tableIndex, uint32_t size) {
    tbl_size = size;
    // PORT FIX (X1-6): a second setup of the same table used to leak the first.
    if (ft_array[tableIndex] != nullptr) {
        sp_ftbl_destroy(&ft_array[tableIndex]);
    }
