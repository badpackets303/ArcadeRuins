...
public:
    ...
    void handle(const S1MIDIEvent &event, S1DSPKernel &kernel);
    ...
    /// PORT FIX (X2-10, ADR-081): the kernel has just been prepared again — `init` has dropped
    /// every voice and every held note — so the keys, the pedal and the notes remembered in here
    /// go too. Left as they were, a key held across an audio device change (or a host's sample
    /// rate change) was still "down" in here and silent in the kernel, and its next note-on was
    /// swallowed (`pressAdded`). Settings are not touched. Not the audio thread: nothing renders.
    void kernelWasPrepared() { clear(); }

    /// PORT FIX (X2-6, ADR-077): the host's transport stopped. What CC123 does — hosts send that
    /// on a stop, "but not always", which is why the kernel acts on the transport itself — so
    /// that the keys whose voices the kernel has just released are not still down in here: a key
    /// that is down is not pressed again (`pressAdded`), and its next note-on was silent.
    void transportDidStop(S1DSPKernel &kernel);
