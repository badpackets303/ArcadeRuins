...
class AppSettings: Codable {
    ...
    // Keyboard
    var labelMode = 1
    // PORT (ADR-035, ADR-039): upstream defaults to 2 octaves, shown. On the Mac the keys sit in
    // a strip under the panels at 4 octaves, and there is no Show: `showKeyboard` is always 0.
    var octaveRange = 4
    var darkMode = 0
    var tuningMode = false
    var showKeyboard = 0.0 // 1 show, 0 hide
    var whiteKeysOnly = false
    // PORT (ADR-035): the keybed layout these keyboard settings were saved under. Settings
    // saved before the keybed have none, and take its defaults once.
    var keybedVersion = AppSettings.currentKeybedVersion
    static let currentKeybedVersion = 1
    ...
    init(dictionary: [String: Any]) {
        ...
        // PORT (ADR-035): an octave range or keyboard visibility saved before the keybed is
        // replaced by the keybed's defaults, once. A choice saved since then is kept.
        let savedUnderKeybed = (dictionary["keybedVersion"] as? Int ?? 0) >= AppSettings.currentKeybedVersion
        if savedUnderKeybed {
        ...
        // PORT (ADR-039): `showKeyboard` is not read. The Mac has no Show, so a saved "shown",
        // from upstream or from ADR-037's build, would only cover the lower panel.
