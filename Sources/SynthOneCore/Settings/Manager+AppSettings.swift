...
extension Manager {
    ...
    func setDefaultsFromAppSettings() {
        ...
        syncHostMIDISettings()   // PORT (ADR-031): loaded settings reach the plugin's render thread
    ...
    func loadSettingsFromDevice() {
        do {
            // PORT (ADR-041): `.settings`, not `.documents`. Banks and presets are shared with
            // the plugin through the App Group; each product keeps its own settings.
            let retrievedSettingData = try Disk.retrieve("settings.json", from: .settings, as: Data.self)
    ...
    func saveAppSettings() {
        do {
            try Disk.save(appSettings, to: .settings, as: "settings.json")   // PORT (ADR-041)
        } catch {
            AKLog("error saving")
        }
    }
