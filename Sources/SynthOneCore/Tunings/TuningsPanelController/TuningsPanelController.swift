...
class TuningsPanelController: PanelController {
    ...
    override func viewDidLoad() {
        ...
        let bg0 = UIImageView(image: UIImage.synthOne("iPhone_TouchPad_bg"), highlightedImage: nil)
        bg0.frame = tuningContainerView.bounds
        let bg1 = UIImageView(image: UIImage.synthOne("iPhone_TouchPad_bg"), highlightedImage: nil)
        ...
        let backChevronImage = UIImage.synthOne("left-arrow")
        ...
        tuningModel.loadTunings {
            ...
            // TuneUp
            // PORT: upstream casts to the app target's `AppDelegate`, which the
            // framework cannot see. `S1LaunchURLProviding` is the part it wanted.
            if let appDelegate = UIApplication.shared.delegate as? S1LaunchURLProviding {
