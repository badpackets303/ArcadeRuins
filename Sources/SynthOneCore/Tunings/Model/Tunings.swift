...
final class Tunings {
    ...
    func loadTunings(completionHandler: @escaping S1TuningLoadCallback) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            // PORT FIX (P4-6): the three subscripts below assume loading produced at
            // least three banks, and nothing guarantees it. `loadTuningsFromDevice`
            // catches its own errors and leaves `tuningBanks` **empty** — which is
            // exactly what happens in the AUv3, where the sandbox cannot read the
            // app's shared folder (ADR-020). The result was an out-of-range subscript
            // that killed the extension before it could vend anything: `auval` failed
            // with `OpenAComponent: result: 4099`, and the reason was only visible in
            // `~/Library/Logs/DiagnosticReports/SynthOneAU-*.ips` (ADR-021).
            //
            // Falling back to the factory tunings is right for both: they are
            // compiled in, so they work in a sandbox, and a plugin showing the curated
            // set beats a plugin that will not load.
            if self.tuningBanks.count <= self.hexanyTriadIndex {
                AKLog("tuning banks did not load (\(self.tuningBanks.count) of 3) — using factory tunings")
                self.loadTuningFactoryPresets()
            }
