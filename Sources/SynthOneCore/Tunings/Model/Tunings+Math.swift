...
extension Tunings {
    ...
    // PORT FIX (P3-1): both of these mix `Float` and `Double` in one expression,
    // which modern Swift rejects — `d01 * 2 * Double.pi` is `Float * Double`, and
    // `radians01(...) - 0.5 * Double.pi` is the same mistake one level up. This is
    // the fourth narrowing bug of this shape in the port; the first three were in
    // AudioKit 4.9.2 (P1-3), and this one is Synth One's own.
    //
    // The arithmetic is done in `Double` and narrowed once at the end, which is
    // what the casts were reaching for. The maths is unchanged: theta = 2*pi*x - pi/2.
    class func horagram01ToCartesian01(p: CGPoint) -> CGPoint {

        let thetaRadians = CGFloat(Double(radians01(d01: Float(p.x))) - 0.5 * Double.pi) // clockwise
    ...
    class func radians01(d01: Float) -> Float {
        ...
        return Float(Double(d01) * 2 * Double.pi)
    }
