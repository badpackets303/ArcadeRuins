...
@IBDesignable open class KeyboardView: UIView, AKMIDIListener {
    ...
    // PORT (ADR-031): the keys the *host* is holding, in the plugin, drawn lit exactly like
    // `onKeys`. Kept apart from `onKeys` because that set drives this view's own touch logic,
    // and host notes are played on the render thread rather than by this view. Always empty in
    // the standalone.
    var hostOnKeys = Set<MIDINoteNumber>() {
        didSet {
            if hostOnKeys != oldValue { setNeedsDisplay() }
        }
    }
