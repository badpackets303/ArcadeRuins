...
//  Ported from AudioKit 4.9.2's iOS `AKADSRView` (MIT), P3-1. Pure UIKit — no
//  AudioKit dependency at all — so it comes across unchanged. Two of the twelve
//  storyboards instantiate it by name (`customClass="AKADSRView"`), which is why
//  it keeps that name (ADR-009).
...
@IBDesignable open class AKADSRView: UIView {
    ...
    /// Background color
    // PORT: upstream defaults this to `AKStylist.sharedInstance.bgColor` —
    // AudioKitUI's theming singleton, which is not ported. All four ADSR views in
    // Envelopes.storyboard set `bgColor` to clear anyway, so the default is never
