...
public class HeaderViewController: UpdatableViewController {
    ...
    public override func viewDidLoad() {
        ...
        // PORT FIX (P5-4): the title hit area and the dice are plain `UIButton`s, so
        // they never got the ADR-026 treatment — that went class by class and neither
        // has a `customClass`. Under the Mac behavioural style a bare button is drawn
        // as a macOS push button, which put a bordered rectangle around the wordmark.
        view.useDesignedButtonAppearanceThroughout()
        ...
        // PORT (ADR-040): More unlocked bonus presets after a mailing-list sign-up, which this
        // port stubs out, so every click re-added the same 94 presets to BankA. Factory BankA
        // now includes them, and More is hidden.
        morePresetsButton.isHidden = true
