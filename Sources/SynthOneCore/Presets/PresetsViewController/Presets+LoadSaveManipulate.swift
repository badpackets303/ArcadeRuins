...
extension PresetsViewController {
    ...
    func loadBanks() {
        ...
        for bank in conductor.banks {
            ...
            if Disk.exists(fileName, in: .documents) {
                ...
                // PORT (ADR-040): upstream adds the 94 bonus presets to BankA only when the
                // header's More button unlocks them, after a mailing-list sign-up this port
                // stubs out. More is gone, so BankA built from the factory files includes
                // them, which is what `addBonusPresets` did. A saved BankA is loaded as it is.
                if bank.name == "BankA" {
    ...
    func loadFactoryPresets(_ bank: String) {
        if let filePath = Bundle.synthOneCore.path(forResource: bank, ofType: "json") {
    ...
    func parsePresetsFromData(data: Data) {
        ...
        presets += Preset.parseDataToPresets(jsonArray: jsonArray, defaults: conductor.presetDefaults)
    }
   ...
   func upgradePresets(banksToUpdate: [String] = [""]) {
        ...
        for bankName in banksToUpdate {
            if let filePath = Bundle.synthOneCore.path(forResource: bankName, ofType: "json") {
                guard let data = try? NSData(contentsOfFile: filePath, options: NSData.ReadingOptions.uncached) as Data
                    else { return }
                let presetsJSON = try? JSONSerialization.jsonObject(with: data, options: [])
                guard let jsonArray = presetsJSON as? [Any] else { return }
                
                let bundlePresets = Preset.parseDataToPresets(jsonArray: jsonArray, defaults: conductor.presetDefaults)
    ...
    func nextPreset() {
        ...
        // PORT FIX (ADR-038): upstream indexes the bank without checking it holds any presets.
        // With none loaded (the plugin, when it cannot read the shared preset folder), the ▶
        // button crashed the plugin in Logic. With nothing to step to, stay put.
        guard !presetBank.isEmpty else { return }
    ...
    func previousPreset() {
        let presetBank = presets.filter { $0.bank == currentPreset.bank }.sorted { $0.position < $1.position }
        // PORT FIX (ADR-038): the same empty-bank crash as `nextPreset`, and a position past the
        // end of the bank (a preset that is not in the loaded list) indexed past it too.
        guard !presetBank.isEmpty else { return }

        if currentPreset.position > 0 {
            currentPreset = presetBank[min(currentPreset.position, presetBank.count) - 1]
