...
import Foundation
// PORT: `import AudioKit`. This file lives inside SynthOneCore now (P2-4).
import S1Support

/// A source of per-parameter default values, for filling in keys a preset's JSON
/// does not carry.
///
/// PORT: upstream's `init(dictionary:)` reads these off
/// `Conductor.sharedInstance.synth`, a UI singleton that does not exist yet and
/// that a preset has no business knowing about. Passed in instead;
/// `AKSynthOne.presetDefaults` is what the app hands it.
public typealias PresetDefaults = (S1Parameter) -> Double
...
class Preset: Codable {
    ...
    class public func parseDataToPresets(jsonArray: [Any], defaults: PresetDefaults) -> [Preset] {
        var presets = [Preset]()
        for presetJSON in jsonArray {
            if let presetDictionary = presetJSON as? [String: Any] {
                let retrievedPreset = Preset(dictionary: presetDictionary, defaults: defaults)
    ...
    class public func parseDataToPreset(presetJSON: Any, defaults: PresetDefaults) -> Preset {
        if let presetDictionary = presetJSON as? [String: Any] {
            return Preset(dictionary: presetDictionary, defaults: defaults)
        }
        return Preset()
    }
    ...
    // PORT: upstream signature is `init(dictionary:)`, reaching into
    // `Conductor.sharedInstance.synth` for the defaults. The defaults are passed
    // in now — a preset is a value, and it should not need a running synth to
    // decode. The three call sites in `PresetDataManager` (Phase 3) gain a
    // `defaults:` argument.
    init(dictionary: [String: Any], defaults: PresetDefaults) {
        ...
        // PORT: upstream reads these three off the live synth
        // (`s.getPattern(forIndex:)` and friends), so the fallback was "keep
        // whatever the last preset left in the sequencer". Read from the same
        // defaults provider as every other field instead — a preset should decode
        // to the same value whatever happened to be loaded before it.
        //
        // Inert for the shipped banks: every preset JSON carries all three keys,
        // so this path only runs for a hand-written or truncated preset.
        for i in 0..<16 {
            let offset = Int32(i)
            if let pattern = S1Parameter(rawValue: S1Parameter.sequencerPattern00.rawValue + offset) {
                seqPatternNoteDefault.append(Int(p(pattern)))
            }
            if let noteOn = S1Parameter(rawValue: S1Parameter.sequencerNoteOn00.rawValue + offset) {
                seqNoteOnDefault.append(p(noteOn) > 0)
            }
            if let boost = S1Parameter(rawValue: S1Parameter.sequencerOctBoost00.rawValue + offset) {
                seqOctBoostDefault.append(p(boost) > 0)
            }
        }
