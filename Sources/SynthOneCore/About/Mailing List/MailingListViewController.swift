...
class MailingListViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func sharePressed(_ sender: Any) {
        ...
        let items: [Any] = ["Arcade Ruins", URL(string: "https://github.com/badpackets303/ArcadeRuins")!]
        let activityViewController = UIActivityViewController(activityItems: items, applicationActivities: nil)
        activityViewController.excludedActivityTypes = [ UIActivity.ActivityType.airDrop]
  
        // PORT FIX (P4-6): was `UIDevice.current.userInterfaceIdiom`, which returns
        // `.mac` under Optimize Interface for Mac (ADR-023) — so the popover source
        // was never set, and Catalyst throws when a `UIActivityViewController` is
        // presented without one. `Conductor.device` is the *layout* idiom.
        if Conductor.sharedInstance.device == .pad {
