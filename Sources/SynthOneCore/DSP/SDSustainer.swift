...
import Foundation
// PORT: `import AudioKit`.
import S1Support

class SDSustainer {

    // PORT (P4-6): was `AKPolyphonicNode`, the concrete class. It is the protocol
    // now, because the plugin has no node — `S1HostedSynth` satisfies it over the
    // audio unit the host provides. The three requirements are all this class used.
    var instrument: AKPolyphonic
    var keyDown: [Bool]
    var isPlaying: [Bool]
    var pedalIsDown: Bool

    required init(_ node: AKPolyphonic) {
        instrument = node
        keyDown = [Bool]()
        isPlaying = [Bool]()
        for _ in 0 ... 127 {
    ...
    open func play(noteNumber: MIDINoteNumber, velocity: MIDIVelocity) {
        ...
        instrument.play(noteNumber: noteNumber, velocity: velocity, channel: 0)
