...
void S1HostMIDI::handle(const AUMIDIEvent &event, S1DSPKernel &kernel) {
...
void S1HostMIDI::forwardControl(S1DSPKernel &kernel, uint8_t status, uint8_t data1, uint8_t data2) {
    // A local, as `hostTempoDidChange` explains: the argument macro takes an address.
    S1HostMIDIMessage message = {status, data1, data2};
    AEMessageQueuePerformSelectorOnMainThread(kernel.audioUnit->_messageQueue,
                                              kernel.audioUnit.messageRelay,
                                              @selector(hostMIDIControlDidArrive:),
                                              AEArgumentStruct(message),
                                              AEArgumentNone);
}
...
void S1HostMIDI::postKeysIfChanged(S1DSPKernel &kernel) {
    ...
    if (AEMessageQueuePerformSelectorOnMainThread(kernel.audioUnit->_messageQueue,
                                                  kernel.audioUnit.messageRelay,
                                                  @selector(hostHeldKeysDidChange:),
                                                  AEArgumentStruct(keys),
                                                  AEArgumentNone)) {
