...
//  Ported from AudioKit 4.9.2 (MIT) at P3-1, so the MIDI panel and the keyboard
//  compile. Names kept per ADR-009.
//
//  **This is the protocol and the message vocabulary only.** Nothing delivers
//  events to it yet — there is no CoreMIDI client, no endpoint scanning, no
//  virtual port. That is P3-4. Until then `Manager` and `KeyboardView` conform to
//  a protocol nobody calls, which is exactly the state upstream's own code would
//  be in with the MIDI subsystem removed.
