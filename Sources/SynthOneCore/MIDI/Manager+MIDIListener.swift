...
extension Manager: AKMIDIListener {
    ...
    /// The octave shift applied to incoming MIDI, in semitones.
    ///
    /// PORT CHANGE (owner's decision, 2026-09-09): upstream applies **no** shift to
    /// MIDI input — `Octave:` moved only the on-screen keyboard, on the reasoning
    /// that a MIDI note carries its own pitch and transposing it is what the
    /// Transpose control is for. That is defensible on an iPad, where the on-screen
    /// keyboard is the only keyboard. On a Mac it means one control labelled
    /// "Octave" that two of the three input paths ignore.
    ///
    /// So `Octave:` is now one global octave: move it up one and the on-screen keys
    /// relabel C2 to C3 *and* a MIDI keyboard's C2 plays C3.
    ///
    /// It reads `typedOctave` rather than the stepper directly so the typing keyboard
    /// and MIDI cannot drift apart — they are the same number by construction.
    var midiOctaveShift: Int { typedOctave * 12 }

    /// The note this MIDI note should sound, or `nil` if the shift pushes it outside
    /// MIDI's range.
    ///
    /// Out of range is dropped rather than clamped. Clamping would pile several keys
    /// onto note 127, and since `notesFromMIDI` is a *set*, releasing one of them
    /// would silence the rest — a stuck-note bug traded for a vanishing-note one.
    func soundingNote(for noteNumber: MIDINoteNumber) -> MIDINoteNumber? {
        let shifted = Int(noteNumber) + midiOctaveShift
        guard (0...127).contains(shifted) else { return nil }
        return MIDINoteNumber(shifted)
    }
    ...
    public func receivedMIDINoteOn(noteNumber: MIDINoteNumber, velocity: MIDIVelocity, channel: MIDIChannel, portID: MIDIUniqueID? = nil, offset: MIDITimeStamp = 0) {
        ...
        // PORT FIX (ADR-032, the owner's decision): upstream played every MIDI note at 127 unless
        // "Velocity Sensitive" was switched on, and shipped with it off. Velocity sensitivity is now
        // always on, here and in the plugin, and the switch is locked on.

        let play = { [self] in
            guard let sounding = soundingNote(for: noteNumber) else { return }
            // Remembered so the note-off undoes *this* shift even if the octave moves
            // while the key is held.
            soundingMIDINotes[noteNumber] = sounding
            keyboardView.pressAdded(sounding, velocity: newVelocity)
            notesFromMIDI.insert(sounding)
        }
    ...
    public func receivedMIDINoteOff(noteNumber: MIDINoteNumber, velocity: MIDIVelocity, channel: MIDIChannel, portID: MIDIUniqueID? = nil, offset: MIDITimeStamp = 0) {
        ...
        DispatchQueue.main.async {
            // The note this key started, not the one it would start now: the octave
            // may have moved while it was held. Falling back to the current shift
            // covers a note-off with no matching note-on, which a host or a hot-
            // plugged device can produce.
            let sounding = self.soundingMIDINotes.removeValue(forKey: noteNumber)
                ?? self.soundingNote(for: noteNumber)
            guard let sounding else { return }

            self.keyboardView.pressRemoved(sounding)
            self.notesFromMIDI.remove(sounding)

            // Mono Mode
            if !self.keyboardView.polyphonicMode {
                let remainingNotes = self.notesFromMIDI.filter { $0 != sounding }
    ...
    public func receivedMIDISetupChange() {
        AKLog("midi setup change, midi.inputNames: \(S1MIDI.shared.inputNames)")
        
        let midiInputNames = S1MIDI.shared.inputNames
        midiInputNames.forEach { inputName in
            ...
            if midiInputs.firstIndex(where: { $0.name == inputName }) == nil
            {
                ...
                S1MIDI.shared.openInput(name: inputName)
            }
        }
    }
