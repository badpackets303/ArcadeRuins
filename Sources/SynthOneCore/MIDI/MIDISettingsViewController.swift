...
class MIDISettingsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // PORT FIX (ADR-032, the owner's decision): velocity sensitivity is always on, in the plugin and
        // the standalone, so the switch shows On, dimmed and inert — what a control that cannot change
        // looks like. Upstream: `velocityToggle.value = velocitySensitive ? 1 : 0`.
        velocityToggle.value = 1
        velocityToggle.isUserInteractionEnabled = false
        velocityToggle.alpha = 0.5
    ...
    @IBAction func bufferSizeChanged(_ sender: UISegmentedControl) {
        ...
        do {
            // PORT: upstream is `try AKTry { try AVAudioSession.sharedInstance()
            // .setPreferredIOBufferDuration(...) }`. `AKTry` was AudioKit's Obj-C
            // exception trampoline. Routed through the session protocol instead, so
            // the AUv3 does not reach into the host's session (P2-2).
            try S1AudioSessionProvider.current.setPreferredIOBufferDuration(AKSettings.bufferLength.duration)
...
extension MIDISettingsViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if midiInput.isOpen {
            S1MIDI.shared.openInput(name: midiInput.name)
        } else {
            S1MIDI.shared.closeInput(name: midiInput.name)
        }
