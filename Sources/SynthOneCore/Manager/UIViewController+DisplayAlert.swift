...
extension UIViewController {
    ...
    // PORT (P3-2): upstream calls `SKStoreReviewController.requestReview()`, with a
    // fallback that opens the App Store listing for id1371050497 — AudioKit's
    // listing, not ours. Routed through the platform-services protocol, whose
    // macOS implementation does nothing (ADR-005: local-only, no listing).
    //
    // The body changes; the three call sites — `InfoViewController`,
    // `AboutViewController` and `Manager`'s every-50-launches prompt — do not.
    func requestReview() {
        S1PlatformServicesProvider.current.requestAppStoreReview()
    }
}
